...
public class AntFarmWorldPresets {

    public static final ResourceKey<WorldPreset> CLASSIC = register("classic");

    private static ResourceKey<WorldPreset> register(String name) {
        return ResourceKey.create(Registries.WORLD_PRESET, AntFarm.id(name));
    }
        ...
        return worldDimensions.get(LevelStem.OVERWORLD).flatMap(p_344665_ -> {
            ...
            return switch (chunkgenerator) {
             ...
             //   case DebugLevelSource debuglevelsource -> Optional.of(DEBUG);
                case NoiseBasedChunkGenerator noisebasedchunkgenerator -> Optional.of(CLASSIC);
    ...
    public static WorldDimensions createNormalWorldDimensions(RegistryAccess registry) {
        return registry.registryOrThrow(Registries.WORLD_PRESET).getHolderOrThrow(CLASSIC).value().createWorldDimensions();
    }

    public static LevelStem getNormalOverworld(RegistryAccess registry) {
        return registry.registryOrThrow(Registries.WORLD_PRESET).getHolderOrThrow(CLASSIC).value().overworld().orElseThrow();
    }
    ...
    static class Bootstrap {
        ...
        Bootstrap(BootstrapContext<WorldPreset> context) {
            ...
            this.netherStem = new LevelStem(holder, new AntFarmStripChunkGenerator(MultiNoiseBiomeSource.createFromPreset(reference), holder1,0,1));
            ...
            this.endStem = new LevelStem(holder2, new AntFarmSquareChunkGenerator(TheEndBiomeSource.create(this.biomes), holder3,16));
        }
        ...
        private LevelStem makeAntFarmOverworld(BiomeSource biomeSource, Holder<NoiseGeneratorSettings> settings) {
            return this.makeOverworld(new AntFarmStripChunkGenerator(biomeSource, settings,0,1));
        }
      ...
      //      this.registerOverworlds(MultiNoiseBiomeSource.createFromPreset(reference));
            ...
            registerCustomOverworldPreset(CLASSIC,makeAntFarmOverworld(MultiNoiseBiomeSource.createFromPreset(reference),holder));
